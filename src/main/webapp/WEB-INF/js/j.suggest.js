//download by http://www.codefans.net
	(function($) {
		...
		$.suggest = function(input, options) {
			...
			//if($.trim($input.val())=='' || $.trim($input.val())=='Choose one or type new') $input.val('Choose one or type new').css('color','#aaa');
			...
			$input.focus(function(){
				if($.trim($(this).val())=='Choose one or type new'){
			...
			function displayItems(items) {
				...
				if (items=='') {//热门城市遍历
					...
					html='<div class="gray ac_result_tip">Choose one or type new</div><ul>'+html+'</ul>';
				}
				else {
					/*if (!items)
					return;
